...
write.csv(comp, file = "Phan/PhanData/PhanCompPaleocoord.csv")
...
write.csv(comp, file = "Phan/PhanData/PhanCompWithTemp.csv")
...
# Reconstruct paleocoordinates 
comp <- read.csv("PhanData/assignLatLonSite/PhanCompUpdated_v2.csv")
paleocoord <- array(dim = c(length(comp$d13C), 2))
...
write.csv(comp, file = "PhanData/PhanCompPaleocoord_v2.csv")

# Extract temps from CESM netCDF
comp <- read.csv("PhanData/PhanCompPaleocoord_v2.csv")
ncin <- nc_open("PhanData/High_Resolution_Climate_Simulation_Dataset_540_Myr.nc")
...
sim.ages <- seq(from = 540, to = 0, by = -10)
GMST_PhanDA <- read.csv("PhanData/PhanDA_GMST.csv")
...
write.csv(comp, "PhanData/PhanCompWithTemp_v2.csv", row.names = FALSE)
