model {
  ...
  bf.nsb_sigma ~ dunif(0, bf.nsb.sd)         
  bf.nsb_tau <- 1 / (bf.nsb_sigma^2)
  bulk.nsb_sigma ~ dunif(0, bulk.nsb.sd)         
  bulk.nsb_tau <- 1 / (bulk.nsb_sigma^2)
  micrite.nsb_sigma ~ dunif(0, micrite.nsb.sd)         
  micrite.nsb_tau <- 1 / (micrite.nsb_sigma^2)
  bulk_sr.nsb_sigma ~ dunif(0, bulk_sr.nsb.sd)         
  bulk_sr.nsb_tau <- 1 / (bulk_sr.nsb_sigma^2)
  bulk_marg.nsb_sigma ~ dunif(0, bulk_marg.nsb.sd)         
  bulk_marg.nsb_tau <- 1 / (bulk_marg.nsb_sigma^2)

  # Site-level NSBs
  for (i in 1:n.sites){
    bf.nsb_site[i] ~ dnorm(bf.nsb.m, bf.nsb_tau)
    bulk.nsb_site[i] ~ dnorm(bulk.nsb.m, bulk.nsb_tau)
    micrite.nsb_site[i] ~ dnorm(micrite.nsb.m, micrite.nsb_tau)
    bulk_sr.nsb_site[i] ~ dnorm(bulk_sr.nsb.m, bulk_sr.nsb_tau)
    bulk_marg.nsb_site[i] ~ dnorm(bulk_marg.nsb.m, bulk_marg.nsb_tau)
  }
