model {
  ...
  # Site-level residuals and NSBs
  for (i in 1:n.sites){
    bf.eta_site[i] ~ dnorm(bf.nsb_mean, bf.nsb_tau)
    pf.eta_site[i] ~ dnorm(pf.nsb_mean, pf.nsb_tau)
    brach.eta_site[i] ~ dnorm(brach.nsb_mean, brach.nsb_tau)
    bivalve.eta_site[i] ~ dnorm(bivalve.nsb_mean, bivalve.nsb_tau)
    amm.eta_site[i] ~ dnorm(amm.nsb_mean, amm.nsb_tau)
    bel.eta_site[i] ~ dnorm(bel.nsb_mean, bel.nsb_tau)
    bulk.eta_site[i] ~ dnorm(bulk.nsb_mean, bulk.nsb_tau)
    micrite.eta_site[i] ~ dnorm(micrite.nsb_mean, micrite.nsb_tau)
    bulk_sr.eta_site[i] ~ dnorm(bulk_sr.nsb_mean, bulk_sr.nsb_tau)
    bulk_marg.eta_site[i] ~ dnorm(bulk_marg.nsb_mean, bulk_marg.nsb_tau)
  ...
  for (i in 1:length(ai.flat)){
    ...
    # Equations 4, 5, and 3 (respectively) of Tipple et al. (2010), from Zhang et al., 1995; bottom water carb chem fractionation
