...
####################################################################################################
####################################################################################################
# Script to generate Figure 2 in the manuscript - 2 panel with timescale strip
...
# ticks every 10 Ma; show numeric labels every 50 Ma
xlim  <- c(540, 0)
...
####################################################################################################
## Second panel: raw d13C colored by paleolat + paleolon, pch by  category
####################################################################################################
par(mar = c(0.2, 5.2, 0.8, 5.2))   
...
prox.in$category_rec[prox.in$category_rec %in% c("bulk marginal sea",
                                                 "bulk marginal sea restricting up section")] <- "bulk carbonate (marginal sea)"
...
lat_bins <- c(0, 30, 60, 90)                 
lon_bins <- c(-180, -90, 0, 90, 180)        
...
# Lat labels (rows) on left of grid 
for (i in seq_len(n_lat)) {
...
# Small symbol legend for recoded category 
legend("bottomright",
       inset = 0.02,
       legend = cat_levels,
